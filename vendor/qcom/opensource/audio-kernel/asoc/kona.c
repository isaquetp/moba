...
static struct snd_soc_dai_link_component tfa9894_spk[] = {
	{
		.name = "tfa98xx.1-0035",
...
#if defined(CONFIG_SND_SOC_AW88264)
...
#if defined(CONFIG_SND_SOC_TFA9894)
		.codecs = tfa9894_spk,
		.num_codecs = ARRAY_SIZE(tfa9894_spk),
#elif defined(CONFIG_SND_SOC_AW88264)
...
codec_aux_dev:
	...
	for (i = 0; i < codec_aux_dev_cnt; i++) {
		...
		dev_err(&pdev->dev,"%s: huanghuanline:%d soc_find_component(aux_codec_of_node, NULL)=%d\n",__func__, __LINE__,soc_find_component(aux_codec_of_node, NULL));
		if (soc_find_component(aux_codec_of_node, NULL)||1) {
	...
	dev_err(&pdev->dev,"%s: huanghuanline:%d codecs_found=%d\n",__func__, __LINE__,codecs_found);
...
aux_dev_register:
	...
	dev_err(&pdev->dev,"%s: huanghuanline:%d \n",__func__, __LINE__);
...
static int msm_asoc_machine_probe(struct platform_device *pdev)
{
	...
	pdata->fsa_handle_2 = of_parse_phandle(pdev->dev.of_node,
					"fsa4480-i2c-handle-2", 0);
	if (!pdata->fsa_handle_2)
		dev_dbg(&pdev->dev, "property %s not detected in node %s\n",
			"fsa4480-i2c-handle-2", pdev->dev.of_node->full_name);
