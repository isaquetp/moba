	...
	*/
	...
	if (tfa98xx->tfa->tfa_family == 2 &&
		...
		if (ret == Tfa98xx_Error_Not_Supported)
			tfa98xx->dsp_fw_state = TFA98XX_DSP_FW_FAIL;
		/*Don't power down device until FW have been loaded*/
		ret = tfa_dev_stop(tfa98xx->tfa);
		tfa98xx->dsp_init = TFA98XX_DSP_INIT_STOPPED;
		mutex_unlock(&tfa98xx->dsp_lock);
	}
...
#endif	
	int ret;
dev_err(codec->dev, "huanghuan_probe:\n");
...
/*	
	tfa98xx->enable_gpio = of_get_named_gpio(np, "enable-gpio", 0);
	if (tfa98xx->enable_gpio < 0)
		dev_dbg(dev, "No enable_gpio GPIO provided,no enable device");*/
	return 0;
}
...
/*	if (gpio_is_valid(tfa98xx->enable_gpio)) {
		gpio_set_value(tfa98xx->enable_gpio, 1);
		gpio_direction_output(tfa98xx->enable_gpio, 1);
		dev_info(tfa98xx->dev, "enable audio power  done\n");
       }
       else {
	   	dev_err(tfa98xx->dev,"enalbe audio power GPIO-%d is invalid \n", tfa98xx->enable_gpio);
       }*/
