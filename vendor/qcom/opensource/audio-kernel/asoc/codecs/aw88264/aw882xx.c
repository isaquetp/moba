...
/*extern int aw_send_afe_cal_apr(uint32_t param_id, void *buf,
	int cmd_size, bool write);*/
 ...
 ******************************************************/
char aw882xx_cfg_name[][AW882XX_CFG_NAME_MAX] = {
	{"aw882xx_spk_reg"},
	{"aw882xx_rcv_reg"},
	{"aw882xx_handfree_reg"},
};

unsigned int aw882xx_mode_cfg_shift[AW882XX_MODE_SHIFT_MAX] = {
	AW882XX_MODE_SPK_SHIFT,
	AW882XX_MODE_RCV_SHIFT,
	AW882XX_MODE_HANDFREE_SHIFT,
};
	...
	unsigned char reg_addr, unsigned char *buf, unsigned int len)
{
	...
	if (ret < 0) {
		aw_dev_err(aw882xx->dev, "%s: i2c master send error, ret=%d\n",
			__func__, ret);
		return ret;
	}
	...
	if (ret != len) {
		aw_dev_err(aw882xx->dev, "%s: couldn't read registers, return %d bytes\n",
			__func__, ret);
		return ret;
	}
			...
			uint8_t db, uint32_t *reg_val)
{
	...
	if (db > (-AW882XX_VOLUME_MIN)) {
		aw_dev_info(aw882xx->dev, "%s: The db is too small, use default\n",
			__func__);
		tmp_val = 0xff;
	} else {
		tmp_val = ((db / AW882XX_VOL_6DB_STEP) << 4) +
				((db % AW882XX_VOL_6DB_STEP));
	}

	*reg_val &= AW882XX_VOL_MASK;

	*reg_val = (tmp_val << AW882XX_VOL_START_BIT) | (*reg_val);

	aw_dev_dbg(aw882xx->dev, "%s: reg_val: 0x%x\n",
			__func__, *reg_val);
	return 0;
}
...
int aw882xx_reg_val_to_db(struct aw882xx *aw882xx,
			uint8_t *db, uint32_t reg_val)
{
	reg_val = reg_val >> AW882XX_VOL_START_BIT;

	*db = (reg_val >> 4) * AW882XX_VOL_6DB_STEP +
		((reg_val & 0x0f) % AW882XX_VOL_6DB_STEP);

	aw_dev_dbg(aw882xx->dev, "%s: db: -%d.%d\n",
			__func__, ((*db) / 2), (((*db) % 2) * 5));
	return 0;
}
	...
	struct aw882xx_container *aw882xx_cont)
{
	...
	for (i = 0; i < aw882xx_cont->len; i += 4) {
		...
		if (reg_addr == AW882XX_HAGCCFG4_REG) {
			aw882xx_reg_val_to_db(aw882xx,
				&aw882xx->monitor.db_offset, (uint32_t)reg_val);
		}
...
static void aw882xx_reg_loaded(const struct firmware *cont, void *context)
{
	...
	aw_dev_info(aw882xx->dev, "%s: loaded %s - size: %zu\n", __func__,
		chan_info->bin_cfg_name[aw882xx->cfg_num], cont ? cont->size : 0);
...
static void aw882xx_get_cfg_shift(struct aw882xx *aw882xx)
{
	...
	if (!chan_info->bin_cfg_name) {
		...
		if (!chan_info->name_suffix) {
			...
			snprintf(chan_info->bin_cfg_name[AW882XX_MODE_HANDFREE_SHIFT],
				AW882XX_CFG_NAME_MAX, "%s.bin",
				chan_info->bin_cfg_name[AW882XX_MODE_HANDFREE_SHIFT]);
		} else {
			snprintf(chan_info->bin_cfg_name[AW882XX_MODE_SPK_SHIFT],
...
{
	aw_dev_info(aw882xx->dev, "%s: flag = %d\n", __func__, flag);
 ...
 *
 ******************************************************/
static const char *const spk_rcv_function[] = { "Spk", "Rcv", "Handfree", "Off"};
...
static int aw882xx_spk_rcv_get(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	struct aw882xx *aw882xx = snd_soc_codec_get_drvdata(codec);
#endif
	int *spk_rcv_control_ptr = &aw882xx->aw882xx_kctrl.aw882xx_spk_rcv_control;

	aw_dev_dbg(aw882xx->dev, "%s: aw882xx_rcv_control=%d\n",
		__func__, *spk_rcv_control_ptr);

	ucontrol->value.integer.value[0] = *spk_rcv_control_ptr;

	return 0;
}

static int aw882xx_spk_rcv_set(struct snd_kcontrol *kcontrol,
	struct snd_ctl_elem_value *ucontrol)
{
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
	struct snd_soc_component *codec = snd_soc_kcontrol_component(kcontrol);
	...
	if (ucontrol->value.integer.value[0] == *spk_rcv_control_ptr)
		return 1;

	*spk_rcv_control_ptr = ucontrol->value.integer.value[0];

	aw882xx->spk_rcv_mode = ucontrol->value.integer.value[0];
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	struct aw882xx *aw882xx = snd_soc_codec_get_drvdata(codec);
        ...
        if(aw882xx->mute_flags){
		aw_dev_dbg(aw882xx->dev, "%s: mute return 0\n",__func__);
		return 0;
	}
	ret = aw_read_data_to_dsp(INDEX_PARAMS_ID_RX_ENBALE,
	...
	struct snd_ctl_elem_value *ucontrol)
{
	...
	ret = aw_read_data_to_dsp(INDEX_PARAMS_ID_TX_ENABLE,
				&ctrl_value, sizeof(uint32_t),
				chan_info->channel);
	if (ret)
		aw_dev_err(aw882xx->dev, "%s: dsp_msg error, ret=%d\n",
		...
		struct snd_ctl_elem_value *ucontrol)
{
	...
	struct aw882xx *aw882xx = snd_soc_codec_get_drvdata(codec);
#endif
	int *pa_switch_ptr = &aw882xx->aw882xx_kctrl.aw882xx_pa_switch;
...
static const struct soc_enum aw882xx_snd_enum[] = {
	SOC_ENUM_SINGLE_EXT(ARRAY_SIZE(spk_rcv_function), spk_rcv_function),
...
static struct snd_kcontrol_new aw882xx_controls[] = {
	SOC_ENUM_EXT("aw882xx_spk_rcv_switch", aw882xx_snd_enum[0],
		aw882xx_spk_rcv_get, aw882xx_spk_rcv_set),
	...
	struct snd_kcontrol_new *src_control, int num)
{
	...
	snd_soc_add_component_controls(aw882xx->codec, dst_control, num);
}

void aw882xx_add_codec_controls(struct aw882xx_kctrl *aw882xx_kctrl)
{
	struct aw882xx *aw882xx = container_of(aw882xx_kctrl,
					struct aw882xx, aw882xx_kctrl);
...
/* add set pa_re to nv
aw_dev_info(aw882xx->dev, "%s: pa_count=%d\n", __func__,pa_count);
if(pa_count<2){
	aw882xx_load_cali_re(&aw882xx->cali);
	pa_count++;
   }
...
static int aw882xx_mute(struct snd_soc_dai *dai, int mute, int stream)
{
	...
	aw882xx->mute_flags = mute;

	if (!(aw882xx->flags & AW882XX_FLAG_START_ON_MUTE))
		return 0;

	if (aw882xx->aw882xx_kctrl.aw882xx_pa_switch == AW882XX_CLOSE_PA){
		aw882xx_smartpa_cfg(aw882xx,false);
		return 0;
	}
 ...
 *****************************************************/
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
static int aw882xx_probe(struct snd_soc_component *codec)
{
	struct aw882xx *aw882xx= snd_soc_component_get_drvdata(codec);
#else
static int aw882xx_probe(struct snd_soc_codec *codec)
{
	...
	int ret = -1;
	struct aw882xx_chan_info *chan_info = &aw882xx->chan_info;
	aw_dev_info(aw882xx->dev, "%s: enter mute=%d\n", __func__,aw882xx->mute_flags);

	aw882xx->codec = codec;

	aw882xx_add_codec_controls(&aw882xx->aw882xx_kctrl);
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
static void aw882xx_remove(struct snd_soc_component *codec)
{
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
static unsigned int aw882xx_codec_read(struct snd_soc_component *codec,
	unsigned int reg)
#else
...
#if LINUX_VERSION_CODE >= KERNEL_VERSION(4,18,0)
static int aw882xx_codec_write(struct snd_soc_component *codec,
...
static void aw882xx_parse_cali_mode_dt(struct aw882xx *aw882xx,
	struct device_node *np)
{
	int ret = -1;
	...
	if (ret < 0) {
		dev_info(aw882xx->dev, "%s: aw-cali-mode get failed ,user default value\n",
				__func__);
		aw882xx->cali.cali_mode = AW_CALI_MODE_NONE;
		goto no_value;
	}

	/*create debugfs node*/
	if (!strcmp(cali_mode_str, "aw_debugfs"))
		aw882xx->cali.cali_mode = AW_CALI_MODE_DBGFS;
	else if (!strcmp(cali_mode_str, "aw_misc"))
		aw882xx->cali.cali_mode = AW_CALI_MODE_MISC;
	else
		aw882xx->cali.cali_mode = AW_CALI_MODE_NONE;

no_value:
	dev_info(aw882xx->dev, "%s:cali mode str:%s num:%d\n",
			__func__, cali_mode_str, aw882xx->cali.cali_mode);
}

static void aw882xx_parse_monitor_dt(struct aw882xx *aw882xx,
	struct device_node *np)
{
	int ret;
		...
		struct device_node *np)
{
	...
	aw882xx_parse_cali_mode_dt(aw882xx, np);
	aw882xx_parse_monitor_dt(aw882xx, np);
	...
	const struct i2c_device_id *id)
{
	...
	ret = snd_soc_register_component(&i2c->dev, &soc_codec_dev_aw882xx,
...
static struct of_device_id aw882xx_dt_match[] = {
	...
	{ .compatible = "awinic,aw882xx_smartpa_left" },
	{ .compatible = "awinic,aw882xx_smartpa_right" },
	{ },
};
