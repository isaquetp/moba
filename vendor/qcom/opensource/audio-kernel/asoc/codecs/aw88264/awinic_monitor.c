...
int aw882xx_monitor_start(struct aw882xx_monitor *monitor)
{
	...
	aw_dev_info(aw882xx->dev, "%s: monitor is_enable %d,spk_rcv_mode %d\n",
				...
				struct aw882xx_low_vol *vol_cfg)
{
	...
	if (aw882xx == NULL || vol_cfg == NULL) {
		aw_dev_err(aw882xx->dev, "%s: pointer is NULL\n", __func__);
		return ret;
	}
...
static void aw882xx_monitor_set_gain(struct aw882xx *aw882xx, uint8_t gain)
{
	...
	gain_reg = gain << AW882XX_BIT_HAGCCFG4_GAIN_SHIFT;
	aw882xx_reg_val_to_db(aw882xx, &gain_db, gain_reg);
	...
	aw882xx_reg_val_to_db(aw882xx, &read_reg_db, reg_val);

	if (read_reg_db == (gain_db + aw882xx->monitor.db_offset)) {
		aw_dev_dbg(aw882xx->dev, "%s: read_reg_gain = -%d.%d, no change\n",
			__func__, (read_reg_db / 2), ((read_reg_db % 2) * 5));
		return;
	}

	aw882xx_db_to_reg_val(aw882xx,
		(gain_db + aw882xx->monitor.db_offset), &reg_val);
	...
	aw_dev_dbg(aw882xx->dev, "%s: set reg val = 0x%x\n",
					__func__, reg_val);
}
static void aw882xx_monitor_set_vmax(struct aw882xx *aw882xx, uint32_t vmax)
{
	...
	ret = aw_read_data_to_dsp(INDEX_PARAMS_ID_RX_VMAX,
	...
	struct device_attribute *attr, char *buf)
{
	struct aw882xx *aw882xx = dev_get_drvdata(dev);
	ssize_t len = 0;
	uint32_t local_enable = aw882xx->monitor.is_enable;
...
void init_aw882xx_monitor(struct aw882xx_monitor *monitor)
{
	...
	if (ret < 0) {
		aw_dev_info(aw882xx->dev, "%s error creating sysfs attr files\n",
			__func__);
	}
}
