					   ...
					   unsigned long mode, void *ptr)
{
	...
	pr_err("headset_analog_num before %d",jzw_4480_headset_ref);
	if (!mbhc)
		return -EINVAL;
	if(jzw_4480_headset_ref == 2){
	...
	}else if(jzw_4480_headset_ref == 1){
					   ...
					   unsigned long mode, void *ptr)
{
	...
	if (!mbhc)
		return -EINVAL;
	if(jzw_4480_headset_ref == 2){
		dev_err(mbhc->component->dev,"second analog headsetmic switch to handset-mic");
		kobject_uevent_env(&mbhc->component->dev->kobj,KOBJ_CHANGE,env1);
	}else if(jzw_4480_headset_ref == 1){
		dev_err(mbhc->component->dev,"second analog headsetmic switch to headsetmic ");
		kobject_uevent_env(&mbhc->component->dev->kobj,KOBJ_CHANGE,env2);
	}
...
int wcd_mbhc_start(struct wcd_mbhc *mbhc, struct wcd_mbhc_config *mbhc_cfg)
{
	...
	const char *usb_c_dt_2 = "qcom,msm-mbhc-usbc-audio-supported-2";
	if (!mbhc || !mbhc_cfg)
		return -EINVAL;
	...
	dev_err(mbhc->component->dev, "%s: huanghuan_wcd_mbhc_start,line=%d\n", __func__,__LINE__);
	...
	if (of_find_property(card->dev->of_node, usb_c_dt_2, NULL)) {
		rc = of_property_read_u32(card->dev->of_node, usb_c_dt_2,
