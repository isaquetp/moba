...
static void resize_f2fs(const std::string& blk_device)
{
    int status;
    int ret;
    const char *resizf2fs_argv[] = {
        F2FS_RESIZE_BIN,
        blk_device.c_str(),
    };

    if (access(F2FS_RESIZE_BIN, X_OK)) {
        LERROR << "Not running " << E2FSCK_BIN << " on " << blk_device << " (executable not in system image)";
    } else {
        ret = android_fork_execvp_ext(ARRAY_SIZE(resizf2fs_argv), const_cast<char **>(resizf2fs_argv),
