...
#if defined(IPT_TLV_SUPPORT)
static nlattr* findNlAttrEx(const nlmsghdr* nh, size_t hdrlen, uint16_t attr) {
 ...
 */
static bool parseTlvMessage(struct nlmsghdr *nh, char **params) {
    bool ret = false;
    unsigned int token;
    struct nlattr *token_attr = findNlAttrEx(nh, sizeof(struct genlmsghdr), TLV_CMD_ATTR_TOKEN);
    if (token_attr && params != NULL) {
        token = nlAttrU32(token_attr);
        asprintf(&params[0], "TOKEN=%d", token);
        ret = true;
    }
    return ret;
}
...
bool NetlinkEvent::parseBinaryNetlinkMessage(char *buffer, int size) {
    ...
    for (nh = (struct nlmsghdr *) buffer;
        ...
        if (!rtMessageName(nh->nlmsg_type)) {
            //ZUIP-4826, yangw12, remove useless log
