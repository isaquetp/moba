...
#define LINE  98 //country number in LocaleSet[][]
#define ITEMS   (LINE*3)
...
"tw","zh","TW",    /* TaiWan */
"sg","en","SG",    /* Singapore */
"sg","en","SG",    /* Myanmar-official */
"mm","my","MM",    /* Myanmar-fork */
"is","it","IT",    /* Italy */
"is","es","ES",    /* Spain */
"is","de","DE",    /* Germany */
"is","fr","FR",    /* France */
"is","en","GB"     /* United Kingdom */
};
...
static void setcc_init_action(int sel)
{
    ...
    LOG(ERROR) << "[EASYIMAGE] setcc_init_action call in with param:" << sel << "\n";
    while(trytimes--) {
        fd = open("/mnt/vendor/persist/flag/countrycode.txt", O_RDONLY);
        if(fd >= 0) {
            findFile = true;
            break;
        } else {
            LOG(ERROR) << "[EASYIMAGE] open /mnt/vendor/persist/flag/countrycode.txt failed! "
                       << strerror(errno);
        }
    }

    if(findFile != true) {
        LOG(ERROR) << "[EASYIMAGE] file[/mnt/vendor/persist/flag/countrycode.txt] not exist!! \n";
        return;
    } else {
        ...
        if(length <= 0) {
            LOG(ERROR) << "[EASYIMAGE] fail to read /mnt/vendor/persist/flag/countrycode.txt! "
                       << strerror(errno);
            return;
        }
    }
    ...
    if(CCode[0] != 0) {
        for(count = 0; count < ITEMS; count = count + 3) {
            if(strncasecmp(CCode, LocaleSet[count], 2) == 0) {
                LOG(ERROR) << "[EASYIMAGE] LocaleSet[" << count << "] "
                           << LocaleSet[count] << ", "
                           << LocaleSet[count+1] << ", "
                           << LocaleSet[count+2] << ".\n";
                ...
                if(sel == 0) {
                    ...
                    LOG(ERROR) << "[EASYIMAGE] set ro properties done. \n";
                } else {
                    property_set("persist.sys.language", LocaleSet[count + 1]);
                    property_set("persist.sys.country", LocaleSet[count + 2]);
                    property_set("persist.log.tag.country.code", CCode);
                    property_set("persist.sys.withsim.country", CCode);
                    property_set("ro.oem.key1", CCode); // Add by wangwq14, ZUIQ-13250, PAI feature.
                    LOG(ERROR) << "[EASYIMAGE] set persist properties done. \n";
                }
                break;
            }
        }
    } else {
        LOG(ERROR) << "[EASYIMAGE] No content in file countrycode.txt \n";
    }
