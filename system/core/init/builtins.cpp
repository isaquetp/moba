...
static Result<Success> do_write_resetdata_misc(const BuiltinArguments& args) {
    LOG(INFO) << "add factory reset";
    int fd = open("/dev/block/bootdevice/by-name/misc", O_RDWR);
    if (fd < 0) {
        return Error() << "Open misc partition failed";
    }

    struct recovery rey_data = {
        "boot-recovery",
        "\0",
        "recovery\n--wipe_data\n--reason=MasterClearConfirm\n--locale=en_US\n"
    };

    WriteFully(fd, &rey_data, sizeof(struct recovery));
    close(fd);
    return Success();
}
...
const BuiltinFunctionMap::Map& BuiltinFunctionMap::map() const {
    ...
    static const Map builtin_functions = {
        ...
        {"factory_data_misc",       {0,     0,    {false,  do_write_resetdata_misc}}},
