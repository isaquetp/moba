...
void adbd_auth_confirm_key(atransport* t) {
    ...
    char adb_debug_on[PROPERTY_VALUE_MAX];
    ...
    property_get("persist.sys.zui.dis_adb_auth", value, "false");
    property_get("ro.boot.adbon", adb_debug_on, "false");
    if (!strcmp(value, "true")
        || !strcmp(adb_debug_on, "true")) {
        if (adb_transport) {
            adbd_auth_verified(adb_transport);
            return;
        }
